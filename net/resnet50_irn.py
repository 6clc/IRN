...
from misc import torchutils, torchutils, indexing
...
class AffinityDisplacementLoss(Net, LightningModule):
    ...
    def __init__(self):
        ...
        self.path_index = indexing.PathIndex(radius=10, default_size=(irn_crop_size // 4, irn_crop_size // 4))
        ...
        self.params = [tuple(self.edge_layers.parameters()),
                tuple(self.dp_layers.parameters())]
    ...
    def training_step(self, batch, batch_idx):
        self.train()
        img = batch['img']
        bg_pos_label = batch['aff_bg_pos_label']
        fg_pos_label = batch['aff_fg_pos_label']
        neg_label = batch['aff_neg_label']

        pos_aff_loss, neg_aff_loss, dp_fg_loss, dp_bg_loss = self(img)
        ...
        logs = {'loss1': pos_aff_loss, 'loss2': neg_aff_loss,
                'loss3': dp_fg_loss, 'loss4': dp_bg_loss}
        

        return {'loss':total_loss, 'logs':logs, 'progress_bar':logs}
    
    def configure_optimizers(self):
        max_step  =  max_step = (10582 // 16) * 5
        optimer = torchutils.PolyOptimizer([
            {'params':self.params[0], 'lr':0.1, 'weight_decay': 1e-4},
            {'params':self.params[1], 'lr':1, 'weight_decay': 1e-4}
        ],
    ...
    def train_dataloader(self):
        ...
        train_dataset = voc12.dataloader.VOC12AffinityDataset(train_list,
                                                          ...
                                                          indices_from=self.path_index.src_indices,
                                                          indices_to=self.path_index.dst_indices,
                                                          hor_flip=True,
    ...
    def make_irn(self, x, crop_size=512, stride=4):
        feat_size = (x.size(2)-1)//stride+1, (x.size(3)-1)//stride+1

        x = F.pad(x, [0, crop_size-x.size(3), 0, crop_size-x.size(2)])
        edge_out, dp_out = super().forward(x)
